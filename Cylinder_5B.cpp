...
// those for HW05 B
void Cylinder::operator*=(double scalar)
{
	this->scale(scalar);
}

std::ostream& operator<<(std::ostream  &out, Cylinder& other)
{
	out<<"Cylinder height: "<<other.get_height()<<" radius: "<<other.get_radius()<<endl;
	return out;
}

Cylinder operator+(Cylinder c1,Cylinder c2)
{
	return Cylinder(c1.height+c2.height,(c1.radius+c2.radius)/2);
}
